...
export default class RtiPage {

    private guestFirstName: Locator;
    private guestLastName: Locator;
    private guestDoB: Locator;
    ...
    constructor(public page: Page) {
        ...
        this.guestFirstName = this.page.getByRole('textbox', { name: 'First Name *' });
        this.guestLastName = this.page.getByRole('textbox', { name: 'Last Name *' });
        this.guestDoB = this.page.getByRole('textbox', { name: 'Date Of Birth *' });
        this.saveAndContinueButton = this.page.getByRole('button', { name: 'Save and Continue' });

        this.seats = this.page.getByText('Seats');
        this.seatsMessage = this.page.locator('.blte-trip-seating-not-available');
        ...
        this.baggageChecked = this.page.getByRole('group', { name: 'extras item' }).filter({
  has: page.getByRole('heading', { name: 'Baggage', level: 5 })
});
        ...
        this.guestOversizedBag = this.page.locator('input[name="outbound0-passenger_2"]');
        ...
        this.secondOversizedBag = this.page.getByRole('button', { name: 'Oversized Carry-on - $20 / leg' }).nth(1);
        
        this.checkedDropDownMenu = this.page.getByText('Checked baggageFrom$');
        this.guestCheckedBag = this.page.locator('input[name="outbound1-passenger_2"]');
        ...
        this.secondChekcedBag = this.page.getByRole('button', { name: 'Checked - $30 / leg' }).nth(1);
        ...
        this.guestCat = this.page.locator('input[name="outbound0-passenger_2"]');
        ...
        this.addDog = this.page.getByRole('button', { name: 'Under seat - $45 / leg' }).nth(1);
    ...
    async enterGuestFirstName(): Promise<void> {
        await this.guestFirstName.fill(guestCredentials.firstName);
    } 
    async enterGuestLastName(): Promise<void> {
        await this.guestLastName.fill(guestCredentials.lastName);
    }
    async enterGuestDoB(): Promise<void> {
        await this.guestDoB.fill(guestCredentials.doB);
    }
    ...
    async asserSeatsMessage(): Promise<void> {
        await expect(this.seatsMessage).toBeVisible();
    }
    ...
    async assertBaggageHeading(): Promise<void> {
        await expect(this.baggageHeading).toBeVisible();
    }
    ...
    async openGuestOversized(): Promise<void> {
        await this.guestOversizedBag.click();
    }
    ...
    async clickSecondOversized(): Promise<void> {
        await this.secondOversizedBag.click();
    }

    async clickCheckedBagMenu(): Promise<void> {
        await this.checkedDropDownMenu.click()
    }
    async clickGuestCheckedBag(): Promise<void> {
        await this.guestCheckedBag.click();
    }
    ...
    async clickSecondCheckedBag(): Promise<void> {
        await this.secondChekcedBag.click();
    }
    ...
    async assertBaggages(): Promise<void> {
        await expect(this.baggageChecked).toBeVisible();
    }
    ...
    async clickGuestCat(): Promise<void> {
        await this.guestCat.click();
    }
